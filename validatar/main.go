...
func Bind(ctx *fasthttp.RequestCtx, objs interface{}) error {
	...
	for i := 0; i < s.NumField(); i++ {
		...
		if rule == "digit" {
			...
			if !CheckStringCommonDigit(defaultVal) || !CheckStringLenth(defaultVal, int(min), int(max)) {
				return errors.New(msg)
			}
		}else if rule == "sAlpha" {
			if !CheckStringAlpha(defaultVal) || CheckStringLenth(defaultVal, int(min), int(max)) {
				return errors.New(msg)
			}
		}else if rule == "url" {
			...
			if !CheckStringAlnum(defaultVal) || !CheckStringLenth(defaultVal, int(min), int(max)) {
				return errors.New(msg)
			}
		}else if rule == "priv" {
			if !ispriv(rule) {
			...
			if !CheckStringModule(defaultVal) || !CheckStringLenth(defaultVal, int(min), int(max)) {
				return errors.New(msg)
			}
		}else if rule == "plainText" {
			if (!CheckStringNormal(defaultVal) && !CheckStringChn(defaultVal)) || !CheckStringLenth(defaultVal, int(min), int(max)) {
				return errors.New(msg)
			}
		}
